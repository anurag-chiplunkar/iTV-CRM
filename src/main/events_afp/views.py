...
def final_deal(request):
	form = Final_Deal(request.POST or None)
	...
	if request.method == "POST":
		if form.is_valid():
			...
			for i in range(1, 10):
				ref = "EventsAFP" + str(i)
				print(ref)
				obj.reference_no = ref
				obj.save()
			obj.ref_program_name = form.cleaned_data.get('ref_program_name')
			obj.ref_channels = form.cleaned_data.get('ref_channels')
			obj.promos = form.cleaned_data.get('promos')
			obj.slot = form.cleaned_data.get('slot')
			obj.eff_rate = form.cleaned_data.get('eff_rate')

			obj.ref_category_name = form.cleaned_data.get('ref_category_name')
			obj.description = form.cleaned_data.get('description')
			obj.ref_channels = form.cleaned_data.get('ref_channels')
			obj.merit_money = form.cleaned_data.get('merit_money')

			obj.save()
		else:
			print('Invalid form')

	return render(request, 'events_afp/final_deal.html', context)
...
def fct_details(request):
	...
	if request.method == "POST":
		if form.is_valid():
			fct_obj.chan = request.POST.get('channels')
	...
	return render(request,'events_afp/fct.html',context)
